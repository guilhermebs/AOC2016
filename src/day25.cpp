...
void solve_pt1()
{
    ...
    std::string line;
    std::unordered_map<std::string, int> registry{{"a", 2}, {"b", 0}, {"c", 0}, {"d", 0}, {"0", 0}, {"1", 1}};
    ...
    size_t i = 0;
    size_t niter = 0;
    while (i < instructions.size() && niter < 50000)
    {
        niter++;
        ...
        {
            //if (out.size() > 0 && registry[parts[1]] == out.back()) break;
            out.push_back(registry[parts[1]]);
            i++;
        }
    }
    for (auto &[k, v]: registry) std::cout << k << ": " << v << ", ";
