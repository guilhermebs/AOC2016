...
void solve_pt2()
{
    ...
    static std::regex move_pos("move position (\\d+) to position (\\d+)");
    std::smatch sm;
    std::string string("fbgdceah");
    //std::string string("cbeghdaf");
    //std::string string("decab");
    ...
    for (auto &line: lines)
    {
        ...
        } else if (std::regex_match(line, sm, rotate_pos))
        {
            auto l = sm[1].str()[0];
            // Y = (2X + 1) % N
            // if y < X => (2X + 1) = Y + N => X = (Y + N - 1) / 2, x < 4
            // if y < X => (2X + 2) = Y + N => X = (Y + N - 2) / 2, x >= 4
            int pos_after = std::find(string.begin(), string.end(), l) - string.begin();
            int pos_before = 0;
            while (pos_after != (2 * pos_before + 1 + (pos_before >= 4))%string.length())
            {
                pos_before++;
            }
            std::cout << pos_after << ", " << pos_before << std::endl;
            string = rotate_str(string, pos_after - pos_before);
        } else if (std::regex_match(line, sm, reverse_pos))
        {
            auto pos1 = std::stoi(sm[1]);
            auto pos2 = std::stoi(sm[2]);
